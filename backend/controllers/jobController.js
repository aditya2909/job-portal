...
exports.updateJob = async (req, res) => {
  try {
    ...
    if (job.company.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ message: "No authorized to update this job" });
    }
...
exports.deleteJob = async (req, res) => {
  try {
    ...
    if (job.company.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ message: "No authorized to delete this job" });
    }
...
exports.getJobsEmployer = async (req, res) => {
  try {
    ...
    const jobs = await Job.find({ company: userId })
      ...
      .lean(); // leand() maked jobs plain JS objects so we can add new fields
