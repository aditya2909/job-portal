...
const JobSeekerDashboard = () => {
  ...
  const [viewMode, setViewModal] = useState("grid");
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      ...
      if (hasFilters) {
        ...
        fetchJobs(); // fetch all jobs if no filters are applied
      }
    }, 500);
  ...
  const toggleSaveJob = async (jobId, isSaved) => {
    try {
      if (isSaved) {
        ...
        await axiosInstance.get(API_PATHS.JOBS.SAVE_JOB(jobId));
      ...
      console.error("Error updating job status. Please try again.");
  ...
  const applyToJob = async (jobId) => {
    try {
      ...
      console.error("Error applying to job. Please try again.");
