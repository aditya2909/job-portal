...
const JobCard = ({ job, onToggleSave, onClick, onApply, saved, hideApply }) => {
  const { user } = useAuth();
        ...
        {/* {!saved && ( */}
          <>
            {job?.applicationStatus ? (
              ...
              !hideApply && (
                <button
                  className="bg-linear-to-r from-blue-50 to-blue-50 text-sm text-blue-700 hover:text-white hover:bg-blue-600 transition-colors px-6 py-2.5 rounded-xl hover:from-blue-500 hover:to-blue-600 tranition-all duration-200 font-semibold transform hover:-translate-y-0.5"
                  onClick={(e) => {
                    e.stopPropagation();
                    onApply();
                  }}
